...
# router = APIRouter(
#     prefix="/movies", tags=["Movies"], dependencies=[Depends(get_current_user)]
# )
...
# ):
#     base_dir = Path("static/posters")
#     base_dir.mkdir(parents=True, exist_ok=True)
#     file_path = base_dir / poster.filename
...
#         title=title, year=year, poster_url=str(file_path), owner_id=user_id
#     )
...
# ):
#     # total_query = await db.execute(select(func.count(models.Movie.id)))
...
#     # Extract movie objects and total count from window function
#     movies = [row[0] for row in rows]
#     total = rows[0][1]  # total from window function

#     return {"data": movies, "count": total, "totalPages": math.ceil(total / limit)}
...
#             models.Movie.id == movie_id, models.Movie.owner_id == user_id
#         )
#     )
...
#     title: str = Form(...),
#     year: int = Form(...),
#     poster: UploadFile = File(None),  # Poster is optional
#     db: AsyncSession = Depends(get_db),
#     user_id: str = Depends(get_current_user),
# ):
#     # Fetch movie
...
#         with file_path.open("wb") as buffer:
#             shutil.copyfileobj(poster.file, buffer)
#         movie.poster_url = str(file_path)
...
#     user_id: str = Depends(get_current_user),
# ):
#     # Fetch the movie by ID and ensure it belongs to the current user
#     result = await db.execute(
#         select(models.Movie).where(
#             models.Movie.id == movie_id, models.Movie.owner_id == user_id
#         )
